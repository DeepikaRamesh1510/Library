...
extension BooksListViewController: UITableViewDelegate , UITableViewDataSource {
	...
	func checkBookCount(rowCount: Int) {
		if rowCount == 0 {
			...
			if self.searchText.length == 0 {
				...
				message = libraryState == .myLibrary && !isLoadingList ? "The searched book is not available in the library!" : "Press enter to Search!"
			}
	...
	func deleteBookFromMyLibrary(bookId: String?, indexPath: IndexPath) {
		...
		fullBooksViewModel.deleteBook(bookId: bookId) { (error) in
	...
	func scrollViewDidScroll(_ scrollView: UIScrollView) {
		if libraryState == .goodReads {
			...
			if (((scrollView.contentOffset.y + scrollView.frame.size.height) > scrollView.contentSize.height ) && !isLoadingList){
				...
				if libraryState == .goodReads {
					self.getGoodReadsBooks(bySearch: self.searchText)
				}
			}
		}
